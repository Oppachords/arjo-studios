...
const projectsData = [
  {
    ...
    // Generates JANI1.jpg through JANI9.jpg dynamically
...
export default function Works2() {
  ...
  return (
    <section id="works" className="py-24 px-6 md:px-12 bg-stone-50 text-stone-950 relative overflow-hidden transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {/* Massive Dynamic Section Heading */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
          {projectsData.map((project) => (
            ...
            >
              {/* Card Face Image Mask Wrapper */}
              <div className="w-full aspect-square bg-stone-100 overflow-hidden relative border border-stone-200/50 rounded-sm mb-6 shadow-sm">
                <img 
                ...
                {/* Smooth Dark Vignette & Hover Interactive Reveal Overlay */}
                <div className="absolute inset-0 bg-stone-950/0 group-hover:bg-stone-950/40 transition-colors duration-500 flex items-center justify-center p-6">
              ...
              {/* Lower Typographic Content Architecture */}
              <div className="flex flex-col">
                <h3 className="text-2xl font-black tracking-tight text-stone-950 uppercase leading-none">
                  {project.title}
                </h3>
                <p className="mt-3 text-sm text-stone-500 font-semibold tracking-tight leading-relaxed">
                  {project.description}
                </p>
              </div>
            </div>
      ...
      {/* ==========================================================================
         Lightbox Gallery Modal Window System
         ========================================================================== */}
      {activeGallery && (
        <div className="fixed inset-0 z-50 bg-[#121212]/95 backdrop-blur-md flex flex-col justify-between p-6 md:p-12 animate-fadeIn">
          ...
          <div className="w-full flex items-center justify-between border-b border-stone-800 pb-6">
            <div>
              ...
              <p className="text-xs text-stone-500 font-bold uppercase tracking-wider mt-1">
                Displaying {activeGallery.galleryImages.length} asset sheets
              </p>
            </div>
          ...
          {/* Core Scrollable Visual Asset Strip Container */}
          <div className="flex-grow my-8 overflow-y-auto pr-2 custom-scrollbar grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 items-start max-h-[70vh]">
            {activeGallery.galleryImages.map((imgSrc, index) => (
              <div 
                key={index} 
                className="w-full aspect-auto bg-neutral-900 border border-stone-800/60 rounded-sm overflow-hidden group/item shadow-lg"
              >
                <img 
                  src={imgSrc} 
                  alt={`Asset array cell index ${index + 1}`}
                  className="w-full h-full object-contain transition-transform duration-500 group-hover/item:scale-102"
                  onError={(e) => {
                    // Suppresses rendering error frames gracefully if index mappings fluctuate slightly
